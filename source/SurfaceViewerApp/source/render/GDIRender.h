...
namespace SV
{
   ...
   public:
      ...
      void SetCenter(const CG::geo_point& center) override;
      CG::geo_point GetCenter()const override { return _center; }
      void SetScale(double scale) override;
      double GetScale()const override { return _scale; }
      void AddObject(render::object&& obj, bool dynamic)override;
      void AddArc(const CG::geo_point& center, double radius, double beg, double end, double step, unsigned long clr, render::LINE_STYLE conture, unsigned int width, bool dynamic) override;
      bool IsNeedRender(const CG::geo_contour& points)const override;
      ...
      math::point GeoToPixel(const CG::geo_point& pos)const  override;
      CG::geo_point PixelToGeo(const math::point& pos)const override;

      std::vector<SV::id_type> GetStaticIds()const override
      {
         std::vector<SV::id_type> res(_staticRenderObjects.begin(), _staticRenderObjects.end());
      ...
      const CG::geo_point* GetStaticCenterId(SV::id_type id)const override
      {
      ...
      CG::geo_contour_vct GetObjectsInsideScreenPts() override;
   ...
   private:
      ...
      inline std::vector<POINT> toPixels(const CG::geo_contour& pPts, bool isPixels)const
      {
         if (isPixels)
            int t = 0;
   ...
   private:
      ...
      HWND _hwnd;
      double _scale;         //< масштаб pixel/mile
      CG::geo_point _center;
      size_t _width;
      size_t _height;
