...
namespace dbg
{
   ...
   public:
      ...
      DebugInfoNodes& GetSubnodes() { return _subnodes; }

      const dbg::debug_info_node* PrepareRef()
      {
         _ref = { _name.c_str(),
                  colreg::geo_points_ref{ _points.data(), _points.size() },
                  prepareSubnodesRef(),
                  dbg::debug_drawing_info{ _text.empty() ? nullptr : _text.c_str(), _pixSz, _color, _ptsLinkage } };
         return &_ref;
      }
      ...
      void Clear()
      {
         ...
         _name = {};
         _text = {};
         _pixSz = dbg::DEFAULT_PIX_SIZE;
         _color = dbg::DEFAULT_COLOR;
         _ptsLinkage = dbg::DEFAULT_LINKAGE;

         _ref = {};
      }

   private:
      inline dbg::debug_info_node_ref prepareSubnodesRef()
      {
         _subnodesRef.clear();
         _subnodesRef.reserve(_subnodes.size());
         for (DebugInfoNode& node : _subnodes)
            _subnodesRef.push_back(*node.PrepareRef());
         return { _subnodesRef.data(), _subnodesRef.size() };
      }

   private:
      std::wstring _name;

      dbg::dbg_points_vct _points;

      std::string _text = {};
      size_t _pixSz = dbg::DEFAULT_PIX_SIZE;
      size_t _color = dbg::DEFAULT_COLOR;
      dbg::PointsLinkage _ptsLinkage = dbg::DEFAULT_LINKAGE;

      DebugInfoNodes _subnodes;

      dbg::debug_info_node _ref;
      std::vector<dbg::debug_info_node> _subnodesRef;
   };
