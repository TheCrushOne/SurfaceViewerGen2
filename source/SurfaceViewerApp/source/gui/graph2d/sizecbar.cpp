...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    szRet.cx = std::max(m_szMin.cx, szRet.cx);
    szRet.cy = std::max(m_szMin.cy, szRet.cy);
...
CSize CSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
    ...
    m_szFloat.cx = std::max(m_szFloat.cx, m_szMin.cx);
    m_szFloat.cy = std::max(m_szFloat.cy, m_szMin.cy);
...
BOOL CSizingControlBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
    ...
    for (int i = 0; i < arrSCBars.GetSize(); i++)
    {
        ...
        nMinLength += bHorz ? pBar->m_szMin.cx - 2:
            pBar->m_szMin.cy - 2;
        nWidth = std::max((LONG)nWidth, bHorz ? pBar->m_szHorz.cy :
            pBar->m_szVert.cx);
    }
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    m_szHorz.cx = std::max(m_szMin.cx, (LONG) pApp->GetProfileInt(szSection,
        _T("sizeHorzCX"), m_szHorz.cx));
    m_szHorz.cy = std::max(m_szMin.cy, (LONG) pApp->GetProfileInt(szSection,
        _T("sizeHorzCY"), m_szHorz.cy));

    m_szVert.cx = std::max(m_szMin.cx, (LONG) pApp->GetProfileInt(szSection,
        _T("sizeVertCX"), m_szVert.cx));
    m_szVert.cy = std::max(m_szMin.cy, (LONG) pApp->GetProfileInt(szSection,
        _T("sizeVertCY"), m_szVert.cy));

    m_szFloat.cx = std::max(m_szMin.cx, (LONG) pApp->GetProfileInt(szSection,
        _T("sizeFloatCX"), m_szFloat.cx));
    m_szFloat.cy = std::max(m_szMin.cy, (LONG) pApp->GetProfileInt(szSection,
