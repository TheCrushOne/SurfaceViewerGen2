...
cglD3D::cglD3D(const CreateParams &params)
{
  ...
  hRes = m_lpD3D9->GetDeviceCaps(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, &m_caps);
  //m_lpD3D9->
  ...
  // Create the device
  hRes = m_lpD3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, ppParams.hDeviceWindow, nBehaviour | D3DCREATE_PUREDEVICE, 
...
void cglD3D::ResetDevice(D3DPRESENT_PARAMETERS i_params, int i_nBPP)
{
    
    i_params.BackBufferCount = 1;
    i_params.MultiSampleType = D3DMULTISAMPLE_NONE;
    i_params.SwapEffect = D3DSWAPEFFECT_DISCARD;
    i_params.EnableAutoDepthStencil = TRUE; // We want to create depth (z-buffer)
    i_params.AutoDepthStencilFormat = m_formats[i_nBPP].fmtDepthBuffer;
    
    i_params.BackBufferFormat = m_formats[i_nBPP].fmtBackBuffer;
    HRESULT hRes = m_lpD3D9Device->Reset(&i_params);
