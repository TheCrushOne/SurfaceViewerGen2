...
public:
   ...
   {
      Json::Value j;
      j[tag::max] = range.max_val;
      j[tag::min] = range.min_val;
   ...
   static settings::range_data<T> rangeReader(const Json::Value& j)
   {
      ...
      range.max_val = valGetter<T>(j[tag::max]);
      range.min_val = valGetter<T>(j[tag::min]);
...
iSettingsSerializer* CreateJsonSettingsSerializer()
{
