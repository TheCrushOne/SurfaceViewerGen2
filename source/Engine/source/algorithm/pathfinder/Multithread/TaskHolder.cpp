...
std::shared_ptr<research::task_holder_statistic::mcmanager_run_log> TaskHolder::m_stat = std::make_shared<research::task_holder_statistic::mcmanager_run_log>();
...
{
   TaskHolder::m_stat.get()->packet_list.clear();
}
...
void TaskHolder::launchSingleTask(task_unit& task)
{
   ...
   EnterCriticalSection(&critical_inner);
   PathfinderStatistic::CommitStatStamp(research::task_holder_statistic::mcmanager_task_log{
      task.task_index,
      task.unit_index,
      task.shard_index,
      task.holder_idx,
